...
def create_random_task_parameters(dql_model: dict[str, list[str]]):
    return {key: [random.randint(0, 7) for _ in dql_model[key]] for key in dql_model}


def create_task_with_parameters(dql_model: dict[str, list[str]], parameters: dict[str, list[int]]):
    return {key: [parameters[key][i] for i in range(len(dql_model[key]))] for key in dql_model}
...
def simulate_task_adaptation(task_count: int, learner_count: int, dql_model: dict[str, list[str]]):
    ...
    for i in range(task_count):
        for j in range(learner_count):
            task_parameters = create_random_task_parameters(dql_model)
            task = create_task_with_parameters(dql_model, task_parameters)
